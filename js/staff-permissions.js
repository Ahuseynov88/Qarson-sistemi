...
/* ── Rol əsaslı sabit icazə dəstləri (Qarsonlar üçün) ── */
const ROLE_PERMISSIONS = {
  waiter:      ['table.view','table.open','order.create','order.view','order.add_item','bill.print','customer.respond','kitchen.notify','chat.send','note.write'],
  head_waiter: ['table.view','table.open','table.close','table.transfer','order.create','order.view','order.add_item','order.cancel_item','order.discount','bill.print','bill.credit','customer.respond','kitchen.notify','chat.send','note.write','waiter.view'],
  cashier:     ['table.view','table.open','table.close','table.transfer','order.create','order.view','order.add_item','order.cancel_item','order.discount','bill.print','bill.credit','bill.payment_cash','bill.payment_pos','bill.payment_credit','report.daily','customer.respond','kitchen.notify','chat.send','note.write','waiter.view'],
  manager:     ['table.view','table.open','table.close','table.transfer','order.create','order.view','order.add_item','order.cancel_item','order.cancel_all','order.discount','order.refund','bill.print','bill.credit','bill.payment_cash','bill.payment_pos','bill.payment_credit','report.daily','report.weekly','customer.respond','kitchen.notify','chat.send','note.write','waiter.view','waiter.manage','menu.view','staff.view'],
  admin:       ['*']
};
...
function hasPermission(action) {
  ...
  if (user.role === 'staff') return staffHasPermission(user, action);
  // Əvvəlki uyğunluq üçün (heç vaxt çağırılmamalı, amma qalsın)
...
function requirePermission(action) {
  ...
  showToast('🚫 Bu əməliyyat üçün icazəniz yoxdur');
  return false;
}

function getRoleDisplayName(waiterRole) {
  const names = { waiter:'🤵 Adi Qarson', head_waiter:'⭐ Baş Qarson', cashier:'💰 Kassir', manager:'👔 Müdir' };
